...
#define READ        true            //don't think we're using read for PCA & servos
#define WRITE       false
...
} I2C_OPEN_STRUCT_TypeDef;  //this is what you use to set up the settings for the I2C peripheral
...
typedef struct {
    ...
    uint8_t                 deviceReg;  //register from which to read from  *only for reading*??

} I2C_PAYLOAD_TypeDef;  // this sets up weather you're reading/writing and to/from what address (not the i2c peripheral)
