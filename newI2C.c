...
/* Theres a private function designed to test the settings to be used in i2c_open
 * to make sure that all inputs are valid. Each setting has only a few valid possible
 * values, so this function will prevent any bad inputs
 *
 * void i2c_openChecker(EUSCI_B_Type *i2c, I2C_OPEN_STRUCT_TypeDef *i2c_open_struct)
 ...
 */
void bus_clear(EUSCI_B_Type *i2c)       // this function clears & resets the I2C bus if it gets blocked
{
    volatile uint8_t i;
 ...
 */
// the i2c_open_struct is set up in main(or somewhere else) by setting each thing in the struct (line 38-43 in lab5 main)
 ...
 */
//  I2C payload struct sets up weather you're reading/writing and to/from what address (not the i2c peripheral)
//******** you fill in all these values like address 7 data when you call this function*****
...
// think we only use one eUSCI_Bx Module to communicate to the PCA so we only need one IRQ
// but for more modules it looks the same but with B1_ or B2_ or B3_
void EUSCIB0_IRQHandler(void)
{
    __disable_irq();

    // Theres a "RX" if else path for reading from a dependent, like reading from a magnetometer/sensor
    // Theres a "TX" if else path for writing to a dependent, like controling the servos?

 // TX if else path for Transmitting/Writing
   ...
   // does this assign the elements in data array (payload[0]=servo# then payload[1]=countH/L) to the UCBxTXBUF eUSCI_Bx Transmit Buffer?
                    //increase byte count
